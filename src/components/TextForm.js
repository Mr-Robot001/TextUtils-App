...
export default function TextForm(props) {
    ...
    const handleUndoClick =()=>{    
     let newText = text.slice(0, text.length-text.split(" ").length);
       setText(newText);
    }
    ...
    return (
        ...
        <h1>{props.heading}</h1>
            ...
            <button disabled={text.length===0} className= "btn btn-success mx-1 my-2" onClick={handleCopy}> CopyText</button>
            <button disabled={text.length===0} className= "btn btn-success mx-1 my-2" onClick={handleUndoClick}> Undo </button>
            <button disabled={text.length===0} className= "btn btn-danger mx-1" onClick={handleExtraSpaces}> Remove Extra Space </button>
        </div>
